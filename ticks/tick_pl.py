...
live_data = {instrument_token: {'ltp': 0, 'pnl': 0} for instrument_token in subscribed_instruments}
...
def update_live_data():
    while True:
        for instrument_token in subscribed_instruments:
            ...
            # Calculate a simple profit/loss for demonstration purposes
            live_data[instrument_token]['pnl'] = ltp - 1000  # Assuming a base price of 1000
...
def update_gui():
    for instrument_token, data in live_data.items():
        ...
        pnl_label.config(text=f"PnL: {data['pnl']}", fg='green' if data['pnl'] >= 0 else 'red')
